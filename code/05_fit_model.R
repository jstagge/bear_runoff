...
###########################################################################
## Process daily flows to create a time series of monthly observed flow
###########################################################################
...
write_csv(flow_border, path = file.path(write_output_path, "flow_border.csv"))
write_csv(flow_monthly, path = file.path(write_output_path, "flow_monthly.csv"))

#flow_border <- read_csv(file.path(write_output_path, "flow_border.csv"))
#flow_monthly <- read_csv(file.path(write_output_path, "flow_monthly.csv"))

###########################################################################
## Read in values
###########################################################################
#load the rda file
load(file = file.path(write_output_path, "upper_huc_params.rda"))


###########################################################################
## Test a model run
###########################################################################
test_param <- c(750, 700,800,650,1150, 0.7, 1.0, 0.56, 0.85, 1.14)

### Choose to use hargreaves samani for Pet
prism_df <- prism_df %>%
	mutate(pet = pet_har_mm)
	
prism_df <- prism_df %>%
	mutate(month = month(date), year = year(date)) %>%
	mutate(date_pos = as.POSIXct(prism_df$date, tz = "America/Denver"))


upper_basin_model(x=test_param, huc_info = hu_char, clim_data = prism_df, hypso = hypso_quant)


optim_upper_basin(x=test_param, huc_info = hu_char, clim_data = prism_df, hypso = hypso_quant, obs=flow_monthly)
...
lower_bounds <- c(0.0001, 0.01)
lower_bounds <- rep(lower_bounds, each=5)
upper_bounds <- c(3000, 20)
...
### 5 iterations = 1.5 minutes
### 20 iterations  = 1.891

huh2 <- sapply(yup$pareto.set, function(x){unlist(x)}) %>%
 t() %>%
  as_tibble() 


huh <- sapply(yup$pareto.set, function(x){attributes(x)$fitness})  %>%
  t() %>%
  as_tibble() 
...
mutate <- dplyr::mutate
### Only intersted for NSE > 0
huh3 <- huh3%>%
	mutate(rank_nse = rank(-NSE, ties.method = "average")) %>%
	mutate(rank_ann_min = rank(RMSE_ann_min, ties.method = "average")) %>%
	mutate(rank_ann_max = rank(-RMSE_ann_max, ties.method = "average"))  %>% 
	mutate(rank_all = rowSums(select(., rank_nse, rank_ann_min, rank_ann_max))) %>%
	arrange(rank_all) %>%
	filter(NSE > 0)
...
, huc_info = hu_char, clim_data = prism_df, hypso = hypso_quant, obs=flow_monthly)




yup <- ga(type = "real-valued", fitness = upper_basin_model, lower = lower_bounds, upper = upper_bounds, huc_info = hu_char, clim_data = prism_df, hypso = hypso_quant, obs=flow_monthly, maxiter = 30) #, parallel=TRUE)
